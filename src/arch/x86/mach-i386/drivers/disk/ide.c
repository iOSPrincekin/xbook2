...
static unsigned char ide_print_error(device_extension_t *ext, unsigned char err)
{
   	...
   	keprint("IDE:");
   	if (err == 1) {keprint("- Device Fault\n     ");}
   	else if (err == 2) {	/* 其它错误 */
		unsigned char state = in8(ATA_REG_ERROR(ext->channel));
		if (state & ATA_ERROR_AMNF)	{keprint("- No Address Mark Found\n     ");}
		if (state & ATA_ERROR_TK0NF){keprint("- No Media or Media Error\n     ");}
		if (state & ATA_ERROR_ABRT)	{keprint("- Command Aborted\n     ");      	}
		if (state & ATA_ERROR_MCR) 	{keprint("- No Media or Media Error\n     "); }
		if (state & ATA_ERROR_IDNF) {keprint("- ID mark not Found\n     ");      }
		if (state & ATA_ERROR_MC) 	{keprint("- No Media or Media Error\n     "); }
		if (state & ATA_ERROR_UNC)  {keprint("- Uncorrectable Data Error\n     ");}
		if (state & ATA_ERROR_BBK)  {keprint("- Bad Sectors\n     "); }
	} else  if (err == 3) {	/* 读取错误 */
		keprint("- Reads Nothing\n     ");
	} else if (err == 4)  {	/* 写错误 */
		keprint("- Write Protected\n     ");
	} else if (err == 5)  {	/* 超时 */
		keprint("- Time Out\n     ");
	}
	keprint("- [%s %s]\n",
...
{
	keprint(PRINT_DEBUG "dump_ide_channel: ext:%x base:%x irq:%d\n", channel, channel->base, channel->irqno);
}

static void dump_ide_extension(device_extension_t *ext)
{
    keprint(PRINT_DEBUG "dump_ide_extension:  ======================= start\n");
	keprint(PRINT_DEBUG "ext:%x channel:%x drive:%d type:%s \n",
	 	ext, ext->channel, ext->drive,
		ext->type == IDE_ATA ? "ATA" : "ATAPI");

	keprint(PRINT_DEBUG "capabilities:%x command_sets:%x signature:%x\n",
		ext->capabilities, ext->command_sets, ext->signature);
	
	if (ext->info->cmdSet1 & 0x0400) {
		keprint(PRINT_DEBUG "Total Sector(LBA 48):");
	} else {
		keprint(PRINT_DEBUG "Total Sector(LBA 28):");
	}
	keprint("%d\n", ext->size);
    
	keprint(PRINT_DEBUG "Serial Number:");
	
	int i;
	for (i = 0; i < 10; i++) {
		keprint("%c%c", (ext->info->Serial_Number[i] >> 8) & 0xff,
			ext->info->Serial_Number[i] & 0xff);
	}
    keprint("\n");
	keprint(PRINT_DEBUG "Fireware Version:");
	for (i = 0; i < 4; i++) {
		keprint("%c%c", (ext->info->Firmware_Version[i] >> 8) & 0xff,
			ext->info->Firmware_Version[i] & 0xff);
	}
    keprint("\n");
	keprint(PRINT_DEBUG "Model Number:");
	for (i = 0; i < 20; i++) {
		keprint("%c%c", (ext->info->Model_Number[i] >> 8) & 0xff,
			ext->info->Model_Number[i] & 0xff);
	}
    keprint("\n");
	keprint(PRINT_DEBUG "LBA supported:%s ",(ext->info->Capabilities0 & 0x0200) ? "Yes" : "No");
	keprint(PRINT_DEBUG "LBA48 supported:%s\n",((ext->info->cmdSet1 & 0x0400) ? "Yes" : "No"));
    keprint(PRINT_DEBUG "dump_ide_extension: ======================= end \n");
}
#endif
	...
	unsigned short count)
{
	...
	if (rw == IDE_READ) {	
#ifdef DEBUG_DRV
        keprint("PIO read count %d->", count);
#endif
		while (i < count) {
			/* 醒来后开始执行下面代码*/
			if ((error = busy_wait(ext))) {     //  若失败
                keprint(PRINT_ERR "wait driver failed!\n");
				/* 重置磁盘驱动并返回 */
				rest_driver(ext);
				return error;
			}
...
#ifdef DEBUG_DRV
			keprint("lba mode %d num %d io %d %d %d %d %d %d->",
				mode, lba, lbaIO[0], lbaIO[1], lbaIO[2], lbaIO[3], lbaIO[4], lbaIO[5]);
			keprint("rw %d dma %d cmd %x head %d\n",
				rw, dma, cmd, head);
#endif
		/* 等待磁盘控制器处于准备状态 */
		while (!(in8(ATA_REG_STATUS(channel)) & ATA_STATUS_READY)) cpu_idle();
...
static int ide_clean_disk(device_extension_t *ext, sector_t count)
{
	...
	if (!buffer) {
		keprint("mem_alloc for ide buf failed!\n");
		return -1;
	}

	memset(buffer, 0, SECTOR_SIZE *10);

	keprint(PRINT_DEBUG "ide_clean_disk: count%d\n", count);
	while (done < count) {
		/* 获取要去操作的扇区数这里用10作为分界 */
		if ((done + 10) <= count) {
			todo = 10;
		...
		//keprint(PRINT_DEBUG "ide_clean_disk: done %d todo %d\n", done, todo);
...
#ifdef DEBUG_DRV
    keprint(PRINT_DEBUG "ide_read: buf=%x sectors=%d off=%x\n", 
...
static int ide_probe(device_extension_t *ext, int id)
{
    ...
    if (timeout <= 0) {
        keprint(PRINT_NOTICE "[ide]: disk %d maybe not ready or not exist!\n", id);
        irq_unregister(channel->irqno, (void *)channel);
        return -1;
    }
...
static iostatus_t ide_enter(driver_object_t *driver)
{
	...
	keprint(PRINT_INFO "ide_enter: found %d hard disks.\n", disk_foud);
	...
	if (disk_foud > 0) {    
        for (id = 0; id < disk_foud; id++) {
            ...
            if (status != IO_SUCCESS) {
                keprint(PRINT_ERR "ide_enter: create device failed!\n");
                return status;
            }
            /* buffered io mode */
    ...
    #ifdef DEBUG_DRV
            keprint(PRINT_DEBUG "ide_enter: device extension: device name=%s object=%x\n",
...
#ifdef DEBUG_DRV
    keprint(PRINT_DEBUG "ide_driver_func: driver name=%s\n",
        driver->name.text);
#endif
    return status;
}
